...
def sniffing_script(new_window, pcap_file_path, ftp_port, smtp_port, telnet_port):
    script_path = r"C:\Users\user\Desktop\FYP\8.2_Network_Sniffing\testing.py"
    try:
       
        print("PCAP File Path:", pcap_file_path)  # Add this line for debugging
        result = subprocess.run(['python', script_path, pcap_file_path, str(ftp_port), str(smtp_port), str(telnet_port)], text=True, capture_output=True, check=True)

        print("Script output:", result.stdout)

        # Display the output in the Text widget
        output_text = Text(new_window, wrap="word", width=70, height=15)
        output_text.grid(row=7, column=0, columnspan=3, padx=5, pady=5, sticky="nsew")  # Place below the other widgets
...
def open_new_window1():
    ...
    label = tk.Label(new_window, text="Network Sniffing", font=('Helvetica', 14, 'bold'))
    label.grid(row=0, column=0, columnspan=3, padx=5, pady=5, sticky="n")

    def select_file():
        file_path = filedialog.askopenfilename(filetypes=[("PCAP files", "*.pcap"), ("All files", "*.*")])
        if file_path:
            pcap_file_path.set(file_path)

    # PCAP file selection
    pcap_file_path = tk.StringVar()
    tk.Label(new_window, text="PCAP File:").grid(row=1, column=0, padx=5, pady=5, sticky="e")
    tk.Entry(new_window, textvariable=pcap_file_path, width=40).grid(row=1, column=1, padx=5, pady=5, sticky="w")
    tk.Button(new_window, text="Browse", command=select_file).grid(row=1, column=2, padx=5, pady=5, sticky="e")

    # FTP Port
    tk.Label(new_window, text="FTP Port (default 21):").grid(row=2, column=0, padx=5, pady=5, sticky="e")
    ftp_port_entry = tk.Entry(new_window)
    ftp_port_entry.grid(row=2, column=1, padx=5, pady=5, sticky="w")

    # SMTP Port
    tk.Label(new_window, text="SMTP Port (default 25):").grid(row=3, column=0, padx=5, pady=5, sticky="e")
    smtp_port_entry = tk.Entry(new_window)
    smtp_port_entry.grid(row=3, column=1, padx=5, pady=5, sticky="w")

    # Telnet Port
    tk.Label(new_window, text="Telnet Port (default 23):").grid(row=4, column=0, padx=5, pady=5, sticky="e")
    telnet_port_entry = tk.Entry(new_window)
    telnet_port_entry.grid(row=4, column=1, padx=5, pady=5, sticky="w")

    # Create a button in the new window
    button_in_new_window = tk.Button(new_window, text="Start now", command=lambda: sniffing_script(new_window, pcap_file_path.get(), ftp_port_entry.get() or "21", smtp_port_entry.get() or "25", telnet_port_entry.get() or "23"))
    button_in_new_window.grid(row=5, column=0, columnspan=3, pady=10, sticky="nsew")
