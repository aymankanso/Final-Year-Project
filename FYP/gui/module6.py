...
def sniffing_script(new_window, pcap_file, ftp_port, smtp_port, telnet_port):
    script_path = r"C:\Users\user\Desktop\FYP\8.2_Network_Sniffing\module7.py"
    process_pcap(pcap_file, ftp_port, smtp_port, telnet_port, new_window)
...
def open_new_window1():
    ...
    label = tk.Label(new_window, text="Network Sniffing", font=("Helvetica", 24))
    label.pack()

    # Function to browse and select the pcap file
    def browse_file():
        filepath = filedialog.askopenfilename(filetypes=[("PCAP files", "*.pcap")])
        pcap_entry.delete(0, tk.END)
        pcap_entry.insert(tk.END, filepath)

    # Function to start sniffing
    def start_sniffing():
        pcap_file = pcap_entry.get()
        ftp_port = int(ftp_entry.get())
        smtp_port = int(smtp_entry.get())
        telnet_port = int(telnet_entry.get())
        sniffing_script(new_window, pcap_file, ftp_port, smtp_port, telnet_port)

    # GUI components for user input
    pcap_label = tk.Label(new_window, text="PCAP File Path:")
    pcap_label.pack()
    pcap_entry = tk.Entry(new_window)
    pcap_entry.pack()
    browse_button = tk.Button(new_window, text="Browse", command=browse_file)
    browse_button.pack()

    ftp_label = tk.Label(new_window, text="FTP Port:")
    ftp_label.pack()
    ftp_entry = tk.Entry(new_window)
    ftp_entry.pack()

    smtp_label = tk.Label(new_window, text="SMTP Port:")
    smtp_label.pack()
    smtp_entry = tk.Entry(new_window)
    smtp_entry.pack()

    telnet_label = tk.Label(new_window, text="Telnet Port:")
    telnet_label.pack()
    telnet_entry = tk.Entry(new_window)
    telnet_entry.pack()

    # Button to start sniffing
    start_button = tk.Button(new_window, text="Start Sniffing", command=start_sniffing)
    start_button.pack()
